...
class Leaderboard(discord.ui.View):
    ...
    async def update(self):
        try:
            self.userData = []
            for i in database.aprilfools.find():
                if isinstance(i["id"], str):
                    if i["id"][:2] == "lt":
                        self.userData.append((int(i["id"][2:]), i["pts"]))

            self.userData.sort(key=lambda x: x[1], reverse=True)
            out = ""
            for index, item in enumerate(self.userData[self.page*12:(self.page+1)*12]):
                ...
                if isinstance(user, discord.User):
                    res = f"> {placeText} *{userStr}* | `{item[1]}`\n"
    ...
    async def forward(self, interaction: discord.Interaction,
        ...
        for i in database.aprilfools.find():
            if isinstance(i["id"], str):
                if i["id"][:2] == "lt":
                    length += 1
        if self.page >= math.ceil(length/12):
            self.page -= 1
